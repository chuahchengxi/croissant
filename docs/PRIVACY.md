...
Croissaint is built to be local-first. Every feature runs on your Mac, and the app has no Croissaint account, cloud dashboard or Croissaint-operated service of any kind. Nothing you capture, type or copy is ever uploaded.
...
- **No automatic telemetry.** Croissaint gathers no usage stats, crash reports or device identifiers.
...
- **No data selling.** Croissaint never sells personal information.
...
When a feature needs a macOS permission such as Accessibility, Screen Recording or Microphone, that access is used only for the feature it belongs to. Captured content never leaves the Mac. The [permissions guide](PERMISSIONS.md) breaks down each permission.
