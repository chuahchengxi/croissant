...
**If you say no.** These features stay off. Croissaint sees the moment you grant the permission and brings them to life with no relaunch.
...
**If you say no.** Croissaint runs without a peep, and the same information is still right there in the panel and in Settings.
...
**Why it comes up.** Keeping the Mac awake with the lid shut relies on `pmset disablesleep`, which needs administrator rights. So it does not have to ask for your password every time you flip closed lid mode, Croissaint can install a tightly scoped `sudoers` rule that allows only that single command.
...
**Optional.** Yes, and it is a one time choice. The rule is limited to `pmset disablesleep` and nothing else, and it goes away on its own when you uninstall Croissaint or reset things from Settings under Advanced.
...
**Optional.** Yes. macOS records the request when an update first tries to change an installed app, so Croissaint cannot check the status beforehand.
