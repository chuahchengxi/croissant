...
# the fan helper daemon, the login item, TCC permissions, preferences,
# saved state, the app's own data folder and (if present) the password-free
# closed-lid sudoers rule. Leaves no dead entries behind.
# Also clears the pre-rename "Croissaint Utils.app" if it is still around.
set -uo pipefail

BUNDLE="com.croissaint.utils"
APP="/Applications/Croissaint.app"
LEGACY_APP="/Applications/Croissaint Utils.app"

echo "▸ Quitting…"
pkill -x Croissaint 2>/dev/null || true
pkill -x CroissaintUtils 2>/dev/null || true
sleep 0.5
...
detached=1
for candidate in "$APP/Contents/MacOS/Croissaint" "$LEGACY_APP/Contents/MacOS/CroissaintUtils"; do
    if [[ -x "$candidate" ]]; then
...
# check on the setting alone would blame Croissaint for a `pmset disablesleep 1`
...
if (( detached == 0 && sleep_stuck == 0 && sleep_unknown == 0 )); then
    echo "✓ Croissaint fully removed."
    exit 0
fi
if (( detached )); then
    echo "⚠ Croissaint removed, but its fan helper is still registered with the system." >&2
    echo "  Reinstall Croissaint, then use Settings › Advanced to uninstall from inside the app." >&2
fi
if (( sleep_stuck )); then
    echo "⚠ Croissaint removed, but this Mac still has sleep switched off." >&2
...
if (( sleep_unknown )); then
    echo "⚠ Croissaint removed, but whether sleep came back could not be read." >&2
