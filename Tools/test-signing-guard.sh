...
# The v0.2.1 and v0.2.2 releases must match their ad-hoc local installations even
...
for key in SIGNING_CERT_P12 SIGNING_CERT_PASSWORD NOTARY_API_KEY_P8 NOTARY_KEY_ID NOTARY_ISSUER_ID; do
    grep -Fq "$key: \${{ github.ref_name != 'v0.2.1' && github.ref_name != 'v0.2.2' && secrets.$key || '' }}" .github/workflows/release.yml
    [[ "$(grep -c "^[[:space:]]*$key:" .github/workflows/release.yml)" == 1 ]]
done
echo 'PASS: signing guards, duplicate certificate names, force-ad-hoc isolation, incomplete release credentials and v0.2.1/v0.2.2 release mode'
