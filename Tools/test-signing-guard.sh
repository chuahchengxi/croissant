...
# The v0.2.1 release must match the explicitly ad-hoc local installation even
...
for key in SIGNING_CERT_P12 SIGNING_CERT_PASSWORD NOTARY_API_KEY_P8 NOTARY_KEY_ID NOTARY_ISSUER_ID; do
    grep -Fq "$key: \${{ github.ref_name != 'v0.2.1' && secrets.$key || '' }}" .github/workflows/release.yml
    [[ "$(grep -c "^[[:space:]]*$key:" .github/workflows/release.yml)" == 1 ]]
done
echo 'PASS: signing guards, duplicate certificate names, force-ad-hoc isolation, incomplete release credentials and v0.2.1 release mode'
