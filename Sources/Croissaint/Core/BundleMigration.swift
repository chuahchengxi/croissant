...
/// OLD PATH ("/Applications/Croissaint Utils.app"), so right after updating the
/// app is running from a bundle still named "Croissaint Utils.app". We rename
/// that bundle to "Croissaint.app" through a detached helper that runs only after
...
///
/// When we are already running as "Croissaint.app", we instead retire a stray
...
enum BundleMigration {
    private static let oldName = "Croissaint Utils.app"
    private static let newName = "Croissaint.app"
    ...
    private static func renameSelfAndRelaunch(from old: URL) -> Bool {
        ...
        let scriptURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("croissaint-rename-\(pid)-\(UUID().uuidString).sh")
        do {
