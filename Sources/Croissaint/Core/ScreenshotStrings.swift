...
struct ScreenshotFeatureStrings {
    ...
    let hideCroissaintWindowsToggle: String
...
extension ScreenshotFeatureStrings {
    static let enUS = ScreenshotFeatureStrings(
        ...
        sharePrivacyData: "Croissaint sends only the rendered image and the expiration you choose. It does not send your name, account, device identifier or MAC address. On this Mac, it keeps only the link, expiration and private deletion token while the link is active.",
    ...
    static let ptBR = ScreenshotFeatureStrings(
        ...
        sharePrivacyData: "O Croissaint envia somente a imagem renderizada e o prazo que você escolher. Ele não envia seu nome, conta, identificador do aparelho nem endereço MAC. Neste Mac, guarda apenas o link, o prazo e o token privado de exclusão enquanto o link estiver ativo.",
    ...
    static let tr = ScreenshotFeatureStrings(
        ...
        sharePrivacyData: "Croissaint yalnızca işlenmiş görüntüyü ve seçtiğiniz süreyi gönderir. Adınızı, hesabınızı, aygıt tanımlayıcınızı veya MAC adresinizi göndermez. Bu Mac'te bağlantı etkinken yalnızca bağlantı, sona erme zamanı ve özel silme anahtarı saklanır.",
    ...
    static let ru = ScreenshotFeatureStrings(
        ...
        sharePrivacyData: "Croissaint отправляет только готовое изображение и выбранный срок действия. Он не отправляет ваше имя, учётную запись, идентификатор устройства или MAC-адрес. На этом Mac сохраняются только ссылка, срок действия и закрытый токен удаления, пока ссылка активна.",
    ...
    static let es = ScreenshotFeatureStrings(
        ...
        sharePrivacyData: "Croissaint solo envía la imagen procesada y el plazo que elijas. No envía tu nombre, cuenta, identificador del dispositivo ni dirección MAC. En este Mac solo guarda el enlace, el vencimiento y el token privado de eliminación mientras el enlace está activo.",
    ...
    static let de = ScreenshotFeatureStrings(
        ...
        sharePrivacyData: "Croissaint sendet nur das fertige Bild und die gewählte Ablaufzeit. Name, Konto, Gerätekennung und MAC-Adresse werden nicht gesendet. Auf diesem Mac bleiben nur der Link, die Ablaufzeit und das private Löschtoken gespeichert, solange der Link aktiv ist.",
    ...
    static let fr = ScreenshotFeatureStrings(
        ...
        sharePrivacyData: "Croissaint envoie uniquement l’image finale et la durée choisie. Il n’envoie ni votre nom, ni votre compte, ni l’identifiant de l’appareil, ni l’adresse MAC. Sur ce Mac, seuls le lien, son expiration et le jeton privé de suppression sont conservés tant que le lien est actif.",
    ...
    static let it = ScreenshotFeatureStrings(
        ...
        sharePrivacyData: "Croissaint invia solo l’immagine elaborata e la scadenza scelta. Non invia nome, account, identificatore del dispositivo o indirizzo MAC. Su questo Mac conserva solo il link, la scadenza e il token privato di eliminazione mentre il link è attivo.",
    ...
    static let ja = ScreenshotFeatureStrings(
        ...
        sharePrivacyData: "Croissaint が送信するのは、書き出した画像と選択した有効期限だけです。氏名、アカウント、デバイス識別子、MACアドレスは送信しません。このMacには、リンクが有効な間だけリンク、有効期限、非公開の削除トークンが保存されます。",
    ...
    static let ko = ScreenshotFeatureStrings(
        ...
        sharePrivacyData: "Croissaint은 완성된 이미지와 선택한 만료 시간만 전송합니다. 이름, 계정, 기기 식별자 또는 MAC 주소는 전송하지 않습니다. 이 Mac에는 링크가 활성화된 동안 링크, 만료 시간 및 비공개 삭제 토큰만 보관됩니다.",
    ...
    static let zhHans = ScreenshotFeatureStrings(
        ...
        sharePrivacyData: "Croissaint 只会发送处理后的图片和你选择的有效期，不会发送姓名、账户、设备标识符或 MAC 地址。链接有效期间，这台 Mac 只保存链接、到期时间和私密删除令牌。",
    ...
    static let zhTW = ScreenshotFeatureStrings(
        ...
        sharePrivacyData: "Croissaint 只會傳送處理後的圖片和你選擇的有效期限，不會傳送姓名、帳號、裝置識別碼或 MAC 位址。連結有效期間，這台 Mac 只會保存連結、到期時間和私人刪除權杖。",
    ...
    static let zhHK = ScreenshotFeatureStrings(
        ...
        sharePrivacyData: "Croissaint 只會傳送處理後的圖片同你揀選嘅有效期限，唔會傳送姓名、帳戶、裝置識別碼或 MAC 位址。連結有效期間，呢部 Mac 只會保存連結、到期時間同私人刪除權杖。",
