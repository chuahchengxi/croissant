...
final class AppDelegate: NSObject, NSApplicationDelegate, NSPopoverDelegate, NSWindowDelegate {
    /// The live instance. Under the SwiftUI lifecycle `NSApp.delegate` is
    /// SwiftUI's own wrapper, not this class, so `NSApp.delegate as? AppDelegate`
    /// is nil and every call routed through it silently does nothing.
    ...
    private static let menuBarLog = Logger(subsystem: Bundle.main.bundleIdentifier ?? "croissaint",
                                           category: "menubar")
    ...
    func relaunchApp() {
        ...
        task.arguments = ["-c", "sleep 0.3; /usr/bin/open \"$1\"", "croissaint-relaunch", path]
        try? task.run()
        NSApp.terminate(nil)
    }
