...
final class AppVolumeMixer: ObservableObject {
    ...
    private let buildQueue = DispatchQueue(label: "com.croissaint.utils.mixer", qos: .userInitiated)
    ...
    private let halQueue = DispatchQueue(label: "com.croissaint.utils.mixer.hal", qos: .userInitiated)
    ...
    private static func outputDevices(defaultUID: String?) -> [MixerOutputDevice] {
        ...
        for deviceID in deviceIDs {
            ...
            let name = read(deviceID, kAudioObjectPropertyName, &nameRef)
                ? nameRef as String
                : uid
            guard name != "Croissaint Mixer" else { continue }
...
private final class TapGainEngine: GainEngine {
    ...
    init?(objects: [AudioObjectID], gain: Float, outputDeviceUID: String) {
        ...
        let aggregate: [String: Any] = [
            kAudioAggregateDeviceNameKey: "Croissaint Mixer",
    ...
    private static let rateQueue = DispatchQueue(label: "com.croissaint.utils.mixer.rate",
                                                 qos: .userInitiated)
    ...
    private static let teardownQueue = DispatchQueue(label: "com.croissaint.utils.mixer.teardown",
                                                     qos: .utility)
