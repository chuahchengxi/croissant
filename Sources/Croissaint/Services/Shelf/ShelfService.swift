...
final class ShelfService: ObservableObject {
    ...
    private let tempDir: URL = {
        let id = Bundle.main.bundleIdentifier ?? "com.croissaint.utils"
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("CroissaintShelf", isDirectory: true)
    ...
    private static let persistQueue = DispatchQueue(label: "com.croissaint.utils.shelf-persist",
                                                    qos: .utility)
    ...
    func fileURLsForActions(startingAt item: Item) -> [URL] {
        fileURLs(for: selection.contains(item.id) ? selectedItems() : [item])
    }

    /// The same list for an action that belongs to the whole shelf rather than
    /// one tile (the header's AirDrop chip): the selection when there is one,
    /// every item otherwise.
    func fileURLsForShelfActions() -> [URL] {
        fileURLs(for: selection.isEmpty ? items : selectedItems())
    }

    /// Whether such an action has anything to send. Payload-only, no file
    /// system hits, so a view can ask on every render.
    var hasFileItems: Bool {
        dragItems(for: selection.isEmpty ? items : selectedItems()).contains { entry in
            if case .file = entry.payload { return true }
            return false
        }
    }

    private func fileURLs(for candidates: [Item]) -> [URL] {
