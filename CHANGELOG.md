...
### Changed
- Granted permissions now survive an update. Every release until now was signed ad-hoc, which gives the app a signature that changes with each build, so macOS treated every update as a different app and quietly dropped the Accessibility and Screen Recording you had given it — System Settings went on showing Croissaint ticked while the app itself saw no permission at all, and the feature you had enabled kept asking for one. Releases are signed with a stable certificate from this version on, so a grant lasts. **Updating to 0.1.5 resets those permissions one last time**, because this is the release where the signature changes: grant them once more and they will stay. macOS still warns that the download is damaged — that needs Apple's notarization, which this project does not have — so the one-line command in the release notes below is still needed the first time.
